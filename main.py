...
class LossHistory(keras.callbacks.Callback):
    ...
    def loss_plot(self, loss_type, curve_color, curve_label):
            plt.figure(1)
            iters = range(len(self.val_losses[loss_type]))
            plt.plot(iters, self.val_losses[loss_type], curve_color, label='vloss'+curve_label)
            plt.grid(True)
            plt.xlabel(loss_type)
            plt.ylabel('val_loss')
            plt.legend(loc="upper right")
            plt.figure(2)
            iters = range(len(self.val_acc[loss_type]))
            plt.plot(iters, self.val_acc[loss_type], curve_color, label='vacc'+curve_label)
            plt.grid(True)
            plt.xlabel(loss_type)
            plt.ylabel('val_acc')
...
def train_and_plot(model_struc, x_train, x_test, optimer, curve_color, curve_label):
    # Compile the model
    model_struc.compile(optimizer=optimer,
                        loss='sparse_categorical_crossentropy',
                        metrics=['accuracy'])
    # Train the model
    model_struc.fit(x_train, train_labels,
              batch_size=1000, epochs=10, verbose=2,
              validation_data=(x_test, test_labels),
              callbacks=[history])
    # Evaluate accuracy
    test_loss, test_acc = model_struc.evaluate(x_test, test_labels, verbose=2)
...
train_and_plot(model, train_images_conv, test_images_conv, 'adam', 'r', '(relu)')
